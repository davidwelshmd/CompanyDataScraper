...
def get_financial_metrics(ticker_symbol):
    ...
    # Revenue, EPS & Revenue Per Share Growth Logic
    ...
    if not income.empty:
        ...
        if 'Total Revenue' in income.index and 'Diluted Average Shares' in income.index:
            rev = income.loc['Total Revenue'].dropna()
            shares = income.loc['Diluted Average Shares'].dropna()
            
            # Align indices to ensure we use revenue and shares from the same year
            common_years = rev.index.intersection(shares.index)
            if len(common_years) >= 2:
                rps = rev.loc[common_years] / shares.loc[common_years]
                
                # 1 Year Rev/Share Growth
                v_curr, v_prev = safe_float(rps.iloc[0]), safe_float(rps.iloc[1])
                if v_curr is not None and v_prev and v_prev > 0:
                    data["Rev/Shr Gth 1yr %"] = safe_round(((v_curr / v_prev) - 1) * 100)
                
                # 3 Year Rev/Share Growth (CAGR)
                if len(rps) >= 4:
                    v_curr, v_old = safe_float(rps.iloc[0]), safe_float(rps.iloc[3])
                    if v_curr is not None and v_old and v_old > 0:
                        data["Rev/Shr Gth 3yr %"] = safe_round(((v_curr / v_old)**(1/3) - 1) * 100)
...
if st.button("Analyse Stocks"):
    ...
    processed_tickers = [t if "." in t else t + target_suffix for t in raw_list]
